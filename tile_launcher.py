...
def _scale_and_crop(src: QPixmap, w: int, h: int) -> QPixmap:
    pix = src.scaled(w, h, Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation)
...
def _extract_exe_icon(exe_path: str, w: int, h: int) -> "QPixmap | None":
    """Extrait l icone principale d un .exe via ExtractIconEx (Windows API)."""
    try:
        ...
        icon_size = 48 if max(w, h) >= 48 else 32

        hicon_large = (wintypes.HICON * 1)()
        hicon_small = (wintypes.HICON * 1)()
        n = shell32.ExtractIconExW(exe_path, 0, hicon_large, hicon_small, 1)
        if n == 0:
            return None

        hicon = hicon_large[0] or hicon_small[0]
        if not hicon:
            return None
        ...
        # BITMAPINFOHEADER via ctypes structure (evite les litteraux binaires)
        ...
        bmi.biHeight   = -icon_size   # top-down
        bmi.biPlanes   = 1
        bmi.biBitCount = 32

        ppv     = ctypes.c_void_p()
        hbitmap = gdi32.CreateDIBSection(hdc_mem, bmi, 0, ctypes.byref(ppv), None, 0)
        gdi32.SelectObject(hdc_mem, hbitmap)
        user32.DrawIconEx(hdc_mem, 0, 0, hicon, icon_size, icon_size, 0, None, 3)

        buf_size = icon_size * icon_size * 4
        ...
        if hicon_large[0]: user32.DestroyIcon(hicon_large[0])
        if hicon_small[0]: user32.DestroyIcon(hicon_small[0])

        # BGRA (Windows) -> RGBA
        raw  = bytes(buf)
        rgba = bytearray(buf_size)
        ...
        img = QImage(bytes(rgba), icon_size, icon_size,
                     icon_size * 4, QImage.Format_RGBA8888)
        pix = QPixmap.fromImage(img)
        return _scale_and_crop(pix, w, h) if not pix.isNull() else None

    except Exception as e:
        print(f"Extraction icone exe : {e}")
        return None


def _get_pixmap(icon_path: str, command: str,
                w: int, h: int, label: str, font_fam: str) -> QPixmap:
    """
    Priorite :
      1. icon defini dans le .ini
      2. Icone extraite de l exe (command), si icon vide
      3. Fallback : degrade + initiale
    """
    key = (icon_path, command, w, h)
    if key in _PIX_CACHE:
        return _PIX_CACHE[key]

    # 1. Icone explicite
    if icon_path:
        path = icon_path if os.path.isabs(icon_path) else os.path.join(BASE_DIR, icon_path)
        if os.path.isfile(path):
            src = QPixmap(path)
            if not src.isNull():
                pix = _scale_and_crop(src, w, h)
                _PIX_CACHE[key] = pix
                return pix

    # 2. Icone de l exe
    if command:
        exe = command.strip().strip('"')
        pix = _extract_exe_icon(exe, w, h)
        if pix:
            _PIX_CACHE[key] = pix
            return pix
...
class TileButton(QFrame):
    def __init__(self, data: dict, tile_w: int, tile_h: int,
        ...
        self.icon_lbl.setPixmap(
            _get_pixmap(data["icon"], data["command"],
                        tile_w, icon_h, data["label"], font_fam)
        )
